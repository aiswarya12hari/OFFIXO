class ProfileModel {
  ...
  factory ProfileModel.fromJson(Map<String, dynamic> json) {
    ...
    /// `branches` lives inside the `member` object in the actual API
    /// response. Parsed defensively — null, missing, empty list, or a
    /// first item without a `name` key all fall back to an empty string
    /// instead of throwing.
    final branchesList = member['branches'];
    String branchName = '';
    if (branchesList is List && branchesList.isNotEmpty) {
      final firstBranch = branchesList[0];
    ...
    return ProfileModel(
      ...
      proofDocument: member['proof_document'] ?? '',
      isBiometricEnabled: member['is_biometric_enabled'] ?? false,
      faceImage1: member['face_image_1'] ?? '',
      faceImage2: member['face_image_2'] ?? '',
      faceImage3: member['face_image_3'] ?? '',
      faceImage4: member['face_image_4'] ?? '',
      allowManualEntry: member['allow_manual_entry'] ?? false,
      isActive: member['is_active'] ?? false,
      organizationName: organization['name'] ?? '',
      organizationType: organization['organization_type'] ?? '',
      organizationOwner: organization['organization_owner'] ?? '',
  ...
  String get faceImage => faceImage1;
}
