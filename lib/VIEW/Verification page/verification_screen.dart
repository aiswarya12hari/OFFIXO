...
class _VerificationScreenState extends State<VerificationScreen>
  ...
  Future<void> _continueAfterBatterySaverSettings() async {
    ...
    try {
      ...
      if (batterySaverOn) {
        ...
        final dialogResult = await BatterySaverDialog.show(context);

        if (_isStaleAttempt(myGeneration)) {
          return;
        }

        if (dialogResult == BatterySaverDialogResult.openedSettings) {
          // Re-arm so the NEXT resume also gets handled - without this
          // the flow silently dies after the first round-trip to
          // Settings if Battery Saver is still on.
          _waitingForBatterySaverSettings = true;

          debugPrint(
            '[BATTERY SAVER] Opening settings again, waiting for user',
          );

          await BatterySaverService.instance.openBatterySaverSettings();
        } else {
  ...
  Future<void> _captureAndVerify() async {
    ...
    try {
      ...
      if (batterySaverOn) {
        debugPrint('[BATTERY SAVER] ON - showing dialog');
        ...
        if (dialogResult == BatterySaverDialogResult.cancelled) {
          debugPrint('[BATTERY SAVER] User cancelled - still ON');
        ...
        // User chose Open Settings - launch it now (this await genuinely
        // waits for the intent to be handled), then hand off to the
        // lifecycle resume handler to continue the flow when the user
        // comes back.
        _waitingForBatterySaverSettings = true;

        debugPrint('[BATTERY SAVER] Opening settings, waiting for user');
