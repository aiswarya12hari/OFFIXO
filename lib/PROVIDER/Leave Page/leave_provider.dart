...
class LeaveProvider extends ChangeNotifier {
  ...
  bool _isCertificateLoading = false; // GET
  bool _isCertificateUploading = false; // PATCH
  String? _certificateError;
  String? _medicalCertificateUrl;

  bool get isCertificateLoading => _isCertificateLoading;
  bool get isCertificateUploading => _isCertificateUploading;
  String? get certificateError => _certificateError;
  String? get medicalCertificateUrl => _medicalCertificateUrl;
  ...
  Future<void> fetchLeaveTypes({bool forceRefresh = false}) async {
    if (!forceRefresh && _leaveTypes.isNotEmpty) {
      return;
    }
  ...
  // ── GET: fetch medical certificate ───────────────────────────────────────
  Future<void> fetchMedicalCertificate(int leaveRequestId) async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse(ApiConfig.medicalCertificateUrl(leaveRequestId)),
        headers: headers,
      );

      debugPrint('CERTIFICATE FETCH STATUS: ${response.statusCode}');
      debugPrint('CERTIFICATE FETCH BODY: ${response.body}');

      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        _medicalCertificateUrl = data['medical_certificate'];
      } else if (response.statusCode == 404) {
        _medicalCertificateUrl = null;
      } else {
        _certificateError = 'Failed to load certificate';
      }
    } catch (e) {
      if (e is SocketException || e.toString().contains('SocketException')) {
        _certificateError = 'Please check your network and try again';
      } else {
        _certificateError = e.toString().replaceFirst(
  ...
  // ── PATCH: upload medical certificate ────────────────────────────────────
  Future<bool> uploadMedicalCertificate(int leaveRequestId, File file) async {
    ...
    try {
      final headers = await SharedPreferenceService.getAuthHeaders();
      // MultipartRequest sets its own multipart/form-data content-type,
      // so drop any JSON content-type before attaching the auth header.
      ...
      debugPrint('CERTIFICATE UPLOAD STATUS: ${response.statusCode}');
      debugPrint('CERTIFICATE UPLOAD BODY: ${response.body}');
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        debugPrint('CERTIFICATE UPLOAD ERROR RAW: ${response.body}');
        String msg = 'Failed to upload certificate';
        try {
          final data = jsonDecode(response.body);
          if (data is Map) {
            final buffer = StringBuffer();
        ...
        _certificateError = msg.replaceFirst(RegExp(r'^Exception:\s*'), '');
      }
    } catch (e) {
      if (e is SocketException || e.toString().contains('SocketException')) {
        _certificateError = 'Please check your network and try again';
  ...
  // ── Reset certificate state (call when leaving the detail screen) ───────
  void resetCertificateState() {
    _isCertificateLoading = false;
    _isCertificateUploading = false;
    _certificateError = null;
    _medicalCertificateUrl = null;
